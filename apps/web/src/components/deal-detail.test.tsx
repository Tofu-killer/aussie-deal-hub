...
describe("public deal surfaces", () => {
  ...
  it("uses locale-specific slugs in the locale switch when a live detail exposes sibling locales", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: string | URL | Request) => {
        const url = new URL(String(input));
        if (decodeURIComponent(url.pathname) === "/v1/public/deals/zh/big-w-乐高盆景树套装-a-59") {
          return new Response(
            JSON.stringify({
              id: "deal_live_lego_1",
              locale: "zh",
    ...
    expect(detail.getByRole("heading", { name: "Big W 乐高盆景树套装 A$59" })).toBeTruthy();
    expect(
      detail.getByRole("link", { name: "English" }).getAttribute("href"),
    ).toBe("/en/deals/lego-bonsai-tree-for-a-59-at-big-w");
    expect(
      detail.getByRole("link", { name: "中文" }).getAttribute("href"),
    ).toBe("/zh/deals/big-w-乐高盆景树套装-a-59");
    detail.unmount();
  });
