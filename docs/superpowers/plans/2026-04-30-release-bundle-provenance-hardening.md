...
- [x] Add deployment artifact assertions that the deploy workflow validates `bundle_run_id`, downloads an exact reviewed artifact name, installs dependencies inside the extracted bundle, and runs deploy from that bundle root.
- [x] Add script-level tests for a new release-bundle-run validation helper covering successful reviewed runs, rejected non-success runs, rejected non-release-bundle workflows, and import-only behavior.
...
- [x] Implement a Node helper that fetches GitHub Actions run metadata, validates the run provenance, and writes the exact reviewed artifact name into `GITHUB_ENV`.
- [x] Update the deploy workflow to use that helper before artifact download, fetch the exact reviewed artifact, install dependencies inside the downloaded bundle, and run `RELEASE_DEPLOY_ROOT=. pnpm release:deploy` from the bundle root.
...
- [x] Document that the manual deploy workflow only accepts successful `Release bundle` runs and deploys using the downloaded artifact's own scripts/contracts.
- [x] Run focused Vitest coverage for the new helper and deploy workflow contracts, then run fresh `pnpm verify`.
