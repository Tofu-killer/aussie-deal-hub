...
describe("deployment artifacts", () => {
  ...
  it("documents a runtime backup entrypoint and ignores generated dump artifacts", () => {
    ...
    expect(readme).toContain("BACKUP_FILE");
    expect(readme).toContain("exact `.dump` path");
  ...
  it("provides a deploy workflow and local deploy entrypoint for reviewed release bundles", () => {
    ...
    expect(readme).toContain("validates the production-critical runtime keys (`DATABASE_URL`, `REDIS_URL`, `SESSION_SECRET`, `EMAIL_FROM`, `SMTP_HOST`, `SMTP_PORT`, `API_BASE_URL`, and `ADMIN_API_BASE_URL`)");
    expect(readme).toContain("rejects the local placeholder defaults that the compose smoke stack uses");
    expect(readme).toContain("creates a remote PostgreSQL backup under `/srv/aussie-deal-hub/shared/backups/`");
    ...
    expect(readme).toContain("`runtime-backup.txt`");
    ...
    expect(readme).toContain("remote backup artifact path");
    expect(readme).toContain("repoints `/srv/aussie-deal-hub/current` back to the previous release");
    expect(readme).toContain("stops the restored release's `api`, `web`, `admin`, and `worker` services before restoring the captured PostgreSQL backup");
    expect(readme).toContain("restoring the captured PostgreSQL backup with `node scripts/runtime-restore.mjs`");
    expect(readme).toContain("starts that restored release again");
    expect(readme).toContain("stores that rollback restore evidence as `rollback-restore.txt`");
