...
| `API_BASE_URL` | web | yes | Server-side base URL that web uses to call the API. Missing values now fail fast instead of falling back to localhost. |
...
| `ADMIN_API_BASE_URL` | admin | yes | Base URL that admin uses to call the API. Missing values now fail fast instead of falling back to localhost. |
...
The script requires `pg_dump` in `PATH`, expands `DATABASE_URL` into libpq runtime settings without placing credentials on the `pg_dump` command line, and writes a custom-format dump into `backups/` by default. Override the destination directory with `BACKUP_DIR`, the filename prefix with `BACKUP_PREFIX`, or the exact `.dump` path with `BACKUP_FILE` when you need a different runtime layout.
...
Run `pnpm release:rehearse` from the repo root to resolve the newest staged bundle under `release/`, reinstalls workspace dependencies there, boots the staged stack with `docker compose up -d --build`, and reruns `smoke:container-health`, `smoke:readiness`, and `smoke:routes` against the same explicit runtime target contract used by `pnpm runtime:verify`. Set `RUNTIME_API_BASE_URL`, `RUNTIME_WEB_BASE_URL`, and `RUNTIME_ADMIN_BASE_URL` for the staged stack, or provide the fully resolved `API_*`, `WEB_*`, `ADMIN_*`, and `WORKER_RUNTIME_URL` targets directly when a rehearse environment exposes different paths. The rehearse build also requires `NEXT_PUBLIC_SITE_URL` so Docker compose cannot silently fall back to localhost SEO origins, and the script reuses that value for `SITE_URL` when `SITE_URL` is omitted. The script dumps compose logs on failure before tearing the stack back down. Override the bundle root with `RELEASE_REHEARSE_ROOT` when you want to rehearse a specific extracted artifact directory.
...
The same workflow then downloads the uploaded artifact into a clean directory, sets `NEXT_PUBLIC_SITE_URL=http://127.0.0.1:3000`, `SITE_URL=http://127.0.0.1:3000`, `RUNTIME_API_BASE_URL=http://127.0.0.1:3001`, `RUNTIME_WEB_BASE_URL=http://127.0.0.1:3000`, and `RUNTIME_ADMIN_BASE_URL=http://127.0.0.1:3002`, and runs `RELEASE_REHEARSE_ROOT=. pnpm release:rehearse` inside that extracted bundle so the uploaded deployment artifact itself is what gets rebuilt and smoke-tested.
...
The script resolves the staged bundle from `RELEASE_DEPLOY_ROOT` or, when unset, the newest bundle under `release/`. It then copies that reviewed artifact to `/srv/aussie-deal-hub/releases`, expects the shared runtime env file at `/srv/aussie-deal-hub/shared/.env.production`, requires that shared env file to define either `NEXT_PUBLIC_SITE_URL` or `SITE_URL`, validates the production-critical runtime keys (`DATABASE_URL`, `REDIS_URL`, `SESSION_SECRET`, `EMAIL_FROM`, `SMTP_HOST`, `SMTP_PORT`, `API_BASE_URL`, and `ADMIN_API_BASE_URL`), rejects the local placeholder defaults that the compose smoke stack uses, creates a remote PostgreSQL backup under `/srv/aussie-deal-hub/shared/backups/`, flips `/srv/aussie-deal-hub/current` to the new release, runs `docker compose --env-file ... up -d --build` remotely, and finishes by running `pnpm runtime:verify` against the supplied runtime base URLs.

If activation or post-deploy runtime verification fails after `current` has been switched, the script captures remote compose status and compose logs for the failing stack, writes local failure diagnostics to `artifacts/release-deploy/<release-name>/` with `metadata.json`, `runtime-backup.txt`, `runtime-verify.txt`, `compose-ps.txt`, `compose-logs.txt`, and `deploy-error.txt`, records the runtime-verify target URLs, `failureStage`, and remote backup artifact path in that metadata, repoints `/srv/aussie-deal-hub/current` back to the previous release when one exists, stops the restored release's `api`, `web`, `admin`, and `worker` services before restoring the captured PostgreSQL backup with `node scripts/runtime-restore.mjs`, starts that restored release again, reruns `pnpm runtime:verify`, stores that rollback restore evidence as `rollback-restore.txt`, stores that rollback verification output as `rollback-runtime-verify.txt`, and still exits non-zero with the original deployment failure. If the rollback itself fails, the script still exits non-zero and surfaces both the original deployment failure and the rollback failure. Override the local diagnostics root with `RELEASE_DEPLOY_DIAGNOSTICS_ROOT` when you want those artifacts somewhere else.
