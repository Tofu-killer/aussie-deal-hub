...
The worker runtime readiness target is stricter than the startup grace window: the smoke only passes after `/v1/admin/runtime/worker` reports `ok: true` with `status: "ok"`, so `status: "starting"` does not count as a verified deploy.
